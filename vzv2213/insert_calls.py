...
cursor.execute("""CREATE TABLE calls (allocation text, expected_calldate date, initials text,
			show_calldate text,
                    expected_calldate_sql date, calltype text, actual_calldate
                    text, actual_calldate_sql date, phonenumber text,
                    call_check_no text, call_check_amt text, email text)""")
connection.commit()

csvre = csv.reader(open("calls.csv", 'rb'), delimiter = ",", quotechar='"')
